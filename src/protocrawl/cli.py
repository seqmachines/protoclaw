...
from protocrawl.config import settings
from protocrawl.models import Protocol


@click.group()
def cli() -> None:
    """Protocrawl — sequencing protocol knowledge base."""


async def _ensure_schema() -> None:
    from protocrawl.db.engine import engine
    from protocrawl.db.tables import Base
...
async def _seed(seeds_dir: Path) -> None:
    ...
    from protocrawl.db.engine import async_session
    from protocrawl.db.repositories import create_protocol, get_protocol_by_slug
...
def serve(host: str, port: int) -> None:
    ...
    uvicorn.run("protocrawl.api.app:app", host=host, port=port, reload=True)
...
async def _run_pipeline(sources: Path, dry_run: bool) -> None:
    ...
    try:
        from protocrawl.agents.root_agent import pipeline_agent
...
async def _list_protocols(assay: str | None, limit: int) -> None:
    from protocrawl.db.engine import async_session
    from protocrawl.db.repositories import list_protocols as db_list
...
async def _submit(
    ...
    from protocrawl.db.engine import async_session
    from protocrawl.db.repositories import get_protocol_by_slug
    from protocrawl.services.ingestion import (
...
async def _list_submissions(limit: int) -> None:
    from protocrawl.db.engine import async_session
    from protocrawl.db.repositories import list_submissions as db_list_submissions
    from protocrawl.services.ingestion import serialize_submission
...
async def _prune_protocols(
    ...
    from protocrawl.db.engine import async_session
    from protocrawl.db.repositories import (
...
async def _draft_protocol(
    ...
    from protocrawl.services.ingestion import build_protocol_draft
